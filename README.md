...
## What's New 1.10.6.18
  ...
  * `DrawnUi.Web` (pure .NET WASM): `fonts.AddEmojis()` and `fonts.AddSymbols()`, the same font subsets DrawnUi.Blazor ships (color emoji ~900 KB as `FontEmoji`, arrows/math/symbols ~285 KB as `FontSymbols` / `FontSymbols2`), served from `_content/DrawnUi.Web/fonts/`. A browser has no system fonts, so without them emoji and symbols simply did not draw.
  * Fix `DrawnUi.Web` NuGet consumers failing to link (`undefined symbol: InterceptBrowserObjects`): the WebGL link flag now arrives through `buildTransitive/DrawnUi.Web.targets`, the file NuGet actually imports.
  * Fix images and SVGs with relative sources (`"drawnui.svg"`) never loading on `DrawnUi.Web` and http sources failing on OpenTK: those heads register no `HttpClient`, so `SkiaImageManager` now falls back to its own, and the web host points it at the page base URL.
  * `DrawnUi.OpenTk`: the mouse wheel now scrolls. `DrawnUiWindow` never forwarded it, so a `SkiaScroll` only moved by dragging. The wheel entry point `Canvas.HandleDesktopWheel` moved from the WPF head into the desktop code shared by OpenTK and WPF, and `DesktopGestureHandler.OnMouseWheel` feeds it; `CanvasHost` apps forward `OnMouseWheel` the same way as the other mouse events.
  * `DrawnUi.OpenTk`: the canvas redraws live while the window is being resized, instead of showing the stretched last frame until the mouse is released. Dragging a window edge runs the OS modal size loop (Windows, macOS), which blocks the render loop; `DrawnUiWindow` now draws a frame from the resize and refresh callbacks GLFW still delivers inside that loop. Normal rendering is unchanged.
  * New sample `src/Maui/Samples/MauiPong`: the Pong game on .NET MAUI, a thin host (window, fonts, background, `RescalingCanvas`) over the shared `Pong.Shared` game with the `DrawnUi.Maui.Game` addon, so every head now ships the Hello + Pong pair (OpenTK, WPF, Blazor, WASM, MAUI).
  * Fix `SkiaScroll` jumping to the top the moment a child below the fold started its own drag (DrawnCamera Adjust: a slider dragged in a scrolled list, in a `SkiaGrid` star row, scrolled itself off screen): the first value change re-measured the page, the grid measured its star row provisionally at the full height, the scroll saw content smaller than that transient viewport and snapped its offset to 0 from the measure pass, and nothing guarded it because the scroll itself was not panning. The "nothing to scroll" snap now happens on the draw, against the final measure, like the bounds clamp already did; content that really shrinks to fit still lands on 0.
  ...
  * Fix mouse wheel scrolling travelled LESS the faster the wheel was spun: every notch restarted the `SkiaScroll` animation from the current offset, so a notch arriving mid-animation threw away the rest of the previous move. A notch now adds `WheelLineSize` onto the running scroll's destination, so a fast spin travels N lines; single notches and slow spins are unchanged. All heads.
  * `DrawnUi.Wpf`: `Super.UseDrawnUi().WithSettings(new DrawnUiStartupSettings { ... })`, the same settings class as MAUI: `DesktopWindow` sizes the window hosting the first element, `UseDesktopKeyboard` feeds every key pressed in the window to `KeyboardManager` (as on MAUI Windows/Mac), `Logger` receives `Super.Log`, `Startup` runs once after initialization.
  * `DrawnUi.Wpf.Game`: the game addon for the WPF head (`DrawnGame` base class from `SharedGame`, like `DrawnUi.Maui.Game`, `DrawnUi.OpenTk.Game`, `DrawnUi.Blazor.Game`, `DrawnUi.Wasm.Game`). New sample `src/Wpf/Samples/WpfPong`: the shared Pong game in a WPF window. `DrawnUiElement(Func<Canvas>)` hosts a `Canvas` subclass (the game's `RescalingCanvas`).
  * Fix `DrawnUi.Wpf` accelerated rendering looking jagged during smooth scrolls and animations, two head defects: the frame was painted into the shared texture BEFORE the `D3DImage` lock was taken, so WPF's render thread could copy a half-drawn frame while the next one was being written (mixed frames); it is now drawn between `Lock` and `AddDirtyRect`, the documented protocol. And animations advanced by the wall clock sampled at the composition tick, which lands a few ms early or late around the vsync while the frame is shown at the vsync, so every animated position carried that jitter; they now advance by the tick's `RenderingTime` step (the frame's presentation time, one vsync per frame), bounded to the wall-clock step because WPF's estimate sometimes leaps an extra frame while the ticks keep their cadence (a game ball moved two frames in one displayed frame). Duplicate `Rendering` ticks for one frame are skipped. Third defect, tearing on a moving object: the shared texture is written by ANGLE's Direct3D 11 queue and read by WPF's Direct3D 9 device with no synchronization object between them, and `glFlush` only submitted the frame; the handover now waits for its completion (`glFinish`) before the dirty rect, 1-3 ms for a frame at 60 Hz.
