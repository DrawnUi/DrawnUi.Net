...
public class LayoutsPage : SkiaLayer
{
    ...
    // MAUI's own XAML converters read the same shorthand as the React props ("*, 2*, Auto").
    private static ColumnDefinitionCollection Cols(string definitions) =>
        (ColumnDefinitionCollection)new ColumnDefinitionCollectionTypeConverter().ConvertFromInvariantString(definitions);

    private static RowDefinitionCollection Rows(string definitions) =>
        (RowDefinitionCollection)new RowDefinitionCollectionTypeConverter().ConvertFromInvariantString(definitions);
}
