...
/// <summary>
/// Registers font subsets shipped inside the DrawnUi.Web package
/// (served as static web assets from <c>_content/DrawnUi.Web/fonts/</c>), the same opt-ins as
/// DrawnUi.Blazor. Browsers give WASM no access to OS fonts, so every glyph must come from a
/// downloaded font; these cover the common gaps at minimal transfer cost.
/// Subsets are built by <c>dev/fonts/subset_fonts.py</c>.
/// </summary>
public static class FontCollectionExtensions
{
    private const string ContentRoot = "_content/DrawnUi.Web/fonts/";

    /// <summary>
    /// Registers a color emoji subset (NotoColorEmoji faces+hands tier, ~900 KB,
    /// alias <c>FontEmoji</c>) so common emoji render instead of nothing.
