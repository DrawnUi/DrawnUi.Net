...
`Samples/OpenTkGpuHost` is a real GPU-backed desktop host for `DrawnUi.Net` using OpenTK and an OpenGL-backed `SKSurface`.

Allows us to:

- validate DrawnUI rendering in a native desktop window
- forward live mouse and keyboard input into DrawnUI
- publish the same host pattern for Windows and Linux
- prove that `DrawnUi.Net` can run anywhere a host can provide a compatible Skia surface

See `Samples/OpenTkGpuHost/README.md` for Linux publish details.
...
The harness writes PNG snapshots plus text summaries for every step. The summary now includes the current target, discovered tagged controls, current target type, and editor-specific state for each tagged `SkiaEditor`.# DrawnUi.Net
