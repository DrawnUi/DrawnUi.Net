...
/// React demo's AccessibilityPage.tsx. The WPF head maps the snapshot to UI Automation peers
/// (DrawnUiElementAutomationPeer) and walks it with Tab / Enter.
/// </summary>
                ...
                Content = new SkiaStack
                {
                    ...
                    Children = new List<SkiaControl>
                    {
                        ...
                        Card("Accessibility snapshot (Canvas.AccessibilityManager)",
                            ...
                            new SkiaLabel("WPF head: every node is a UI Automation peer under the canvas pane (role, name, help text, Invoke / Toggle patterns, live regions). Tab / Shift+Tab walk the interactive nodes with a focus ring, Enter or Space activates, Escape leaves.")
                            {
