...
`Gestures = GesturesMode.Enabled` routes browser pointer/touch/wheel input to DrawnUI. A right click (or a long press, or the Menu key) raises `ContextMenu` on the control under it; a handler returning `true` suppresses the browser's own canvas menu, otherwise it shows. Every mouse button still taps, with the button in `e.Parameters.Event.Pointer` — see [Mouse buttons and the browser context menu](../gestures.md#mouse-buttons-and-the-browser-context-menu). If you wire `main.js` by hand, pass `onContextMenu: Input.OnContextMenu` in `setModuleExports`. `GesturesMode.Enabled` shares input with the page like MAUI's `Enabled` inside a native scroll view: a finger pan along an axis the page can scroll scrolls the page (taps and the other axis stay on the canvas, and a page that cannot scroll keeps every touch; inside an iframe the embedding page is assumed to scroll vertically), and the wheel scrolls the page unless a control used it. `GesturesMode.Lock` keeps all input and additionally applies the CSS guard (`touch-action`, `overscroll-behavior`, a non-passive `touchmove` guard) that stops iOS page panning / swipe-to-close while DrawnUI owns the gesture.
