...
## Fixed design viewport, "same look everywhere, only the scale changes"

Author a screen in design units (e.g. 390pt wide) and let the engine fit it: override the RENDERING SCALE of a subtree, never a `Scale` transform (a transform leaves measure/layout at the old size and breaks Fill/End alignment). Two verified helpers, same mechanism (`context.Scale` is a mutable struct field):
- Canvas level: `RescalingCanvas` (DrawnUi `src/Shared/Samples/Pong.Shared/Views/RescalingCanvas.cs`, `LogicalWidth/LogicalHeight`, `context.Scale *= min(fitX, fitY)` in `Draw`). Blazor twin `AspectLockedCanvas` (`src/Blazor/DrawnUi/Views`).
- Layout level, for a screen hosted inside a shell canvas: `RescalingLayout : SkiaLayout` (Absolute) — ArtOfFoto `src/Mobile/Controls/RescalingLayout.cs` (device-verified Windows 2026-09-10, 4 window proportions), BlockChampion `RescalingLayout`. Override `MeasureAbsolute(rect, scale)`: `ContextScale = scale * min(rect.W/(LogicalW*scale), rect.H/(LogicalH*scale))` then `base.MeasureAbsolute(rect, ContextScale)`; override `DrawViews(ctx)`: `ctx.Scale = ContextScale`. Children are measured AND drawn at the fitted scale, get it as their `RenderingScale`, so hit-testing and `GetOffsetInsideControlInPoints` just work. Keep the rescaling layout itself without Padding (its own insets use the parent scale in both `GetMeasuringConstraints` and `Paint`).
- Usage: `RescalingLayout{LogicalWidth=390, LogicalHeight=minDesignHeight}.Fill()` > `SkiaLayer{WidthRequest=390, HorizontalOptions=Center}.FillY()` > top content `.StartY()` + bottom row `.EndY()`. Tall screens: width-driven scale, the gap between top and bottom grows, bottom row stays at the bottom. Short/wide windows: height-driven scale, the 390-wide column centers. Safe-area insets are physical: add them OUTSIDE the rescaled subtree (spacer with `Super.NavigationBarHeight`).
...
## Thin strokes: use 1.55, never 1 (verified 2026-09-12, FiltersCamera Windows)

A positive `SkiaShape.StrokeWidth` is in points (× `RenderingScale`); a negative value means raw pixels. For rounded rectangles (`CornerRadius` set) and circles, `SkiaShape.GetCompensatedStrokeWidth` multiplies every stroke of **1pt or less by 0.55** ("make curves thinner"). So `StrokeWidth = 1` actually draws about 0.55pt. With a low-alpha stroke color it looks like a 1px hairline and almost disappears on some screens. The multiplier stops just above 1: any value over 1 draws at its real width.

- For a very thin outline on chips, pills and chip-like buttons that looks the same on every platform and density, set **`StrokeWidth = 1.55`** (true 1.55pt, no thinning). The app owner approved this look in the Windows desktop build. Other heads were not checked, but the multiplier is shared code.
- Keep the value in ONE app constant (e.g. `AppColors.StrokeWidth`) next to the stroke color, and reference it from every chip-like shape so the look can be tuned in one place.
- Tell for this trap: an outline "looks like 1 pixel, not 1 point" or is nearly invisible at `StrokeWidth = 1` while 2 looks fine.
- Also note the pixel alignment: rounded strokes ≤ 2pt are drawn on a `Round(rect) ± 0.5` pixel grid.
...
- **Single-line `HorizontalTextAlignment=Center/End` (fixed 2026-09-10, lib `SkiaEditor.OnMeasuring`)**: the single-line label is `Start`-aligned inside a horizontal `SkiaScroll` and measures to its content, so Center/End had nothing to align in (the `ViewportConstraintWidth` path in `SkiaLabel.DrawLines` only fires for a HUGE rect, which the 2026-08-28 "Fill on infinite = content" sweep removed). Now the editor gives the label `MinimumWidthRequest` = its own width constraint minus padding/margins when alignment != Start: short text centers in the field, longer text still grows past it and scrolls, caret math unchanged (`labelLine.Bounds`). Placeholder alignment is separate: set `PlaceholderHorizontalAlignment` too. Verified on Windows (ArtOfFoto `SkiaUnderlinedEntry`).
- **Drop-in drawn replacement for a native MAUI entry composite** (ArtOfFoto `src/Mobile/Controls/SkiaUnderlinedEntry.cs`, 2026-09-10): `ContentView` > `Canvas{Gestures=Enabled}` > `SkiaLayer` > [`SkiaStack`(editor + gradient underline `SkiaShape`) , caption `SkiaLabel.EndY()`]; editor with `BackgroundColor=Transparent, StrokeWidth=0` (kills the platform style box), `MaxLines=1`, `KeyboardType` mapped from MAUI `Keyboard`. Keep the OLD control's API (Text, PlaceholderText, EditingEvent, `SetInputFilter(Func<string,int,int,string>)`) so page code-behind is untouched; the Android-style input filter is emulated in `OnTextChanged`: diff old/new text (common prefix/suffix) → run the filter over the inserted run while `Text` still returns the OLD value (filters read it) → revert `editor.Text` when rejected. `EditingEvent` fires only on user edits, never on programmatic `Text` sets (pages recalc themselves after loading settings). Canvas auto-sizes its height from content inside a MAUI StackLayout.
...
- Raw handling: `ConsumeGestures` — handler `(object sender, SkiaGesturesInfo e)`; check `e.Args.Type` (`TouchActionResult`: Touch/Down/Up/Tapped/LongPressing/Panning/Wheel/Pointer/ContextMenu — there is NO `Cancelled`, verified in AppoMobi.Gestures source 2026-09-12; `Up` ends every gesture), set `e.Consumed = true` to stop propagation. Keep the handler SYNCHRONOUS; offload async/animation via `Task.Run` inside.
- **Swipe-to-close cascade (verified 2026-09-12, FiltersCamera Windows):** a handler that closes/pops its screen from `Panning` hands the REST of the same pan to whatever screen is revealed underneath, and that screen's own pan handler fires too if it computes travel from a stale Down start (one swipe closed Credits, then Profile, then stepped the camera filter). Arm every pan-reacting handler on `Down`, disarm on `Up`, and ignore `Panning` while unarmed. Reusable pattern: a `SwipeBackLayer : SkiaLayer` overriding `ProcessGestures` — call `base` first (children keep buttons, switches, vertical scrolls), fire when rightward travel ≥ 80pt and `dx > 2·|dy|`, block for the whole gesture when the consumer is `SkiaScroll { Orientation: not Vertical }`. Test with real pointer moves: `SetCursorPos` drags arrive as a Down + Up (a TAP at the release point, which silently toggled settings rows); use `mouse_event` with `MOVE|ABSOLUTE|VIRTUALDESK` steps between down and up.
...
- **Anti-aliasing is the exception**: pattern sizes (line width, glow, cells) are image-relative, but an edge's blend width must be ~1 OUTPUT pixel: `aa = 0.75 * max(iImageResolution / iResolution)` in texture pixels. Scaling the AA with the image blurs edges on the photo.
- **Posterize / zone look without pixel speckle, cheap enough for realtime video** (measured 2026-09-13 in a SkiaSharp harness on a neon pop-art filter: 82 -> 17 samples per pixel, CPU render 11.4 s -> 1.3 s at 2160x2880, isolated zone-flip pixels at 100% zoom 1.33% -> 0.10%). Look caveat: the app owner REJECTED the ink outlines for pop art, because dark lines between colour zones kill the flat-colour concept. Outlines are a comics/print choice only. For flat-colour looks, blur the colour before posterizing (Comics-style gaussian) and keep zones touching directly:
  - **Pick the zone from smoothed luminance**, never from the raw pixel: a 3x3 binomial of taps spaced `1 * pxPerBase`. Near a threshold, camera noise flips raw pixels between zones, which reads as computer-generated dithering.
  - **Half-texel shift for free denoise**: `p = inputCoord + 0.5` puts every linear-filtered tap between 4 texels, so 9 samples average 36 texels. Needs linear filtering, which is the DrawnUI default.
  - **Border distance from the same taps**: a Sobel gradient on the 3x3 gives luminance per texture pixel. `(lum - threshold) / grad` is the distance to the zone border in pixels. It drives crisp anti-aliased borders (`smoothstep(-aa, aa, distance)`) and ink outlines of fixed image-relative width (`1 - smoothstep(halfWidth - aa, halfWidth + aa, distance)`) with no extra samples.
  - **Guards**: fade the ink by `smoothstep(k*0.5, k, grad * pxPerBase)` so noise and soft gradients get no lines, and give the zone blend a minimum luminance width (`max(aa * grad, 0.02)`) so soft-shading borders do not fray.
  - **Wide soft effects (glow) need few taps**: an 8-tap ring at the radius plus the inner mean replaced an 81-tap grid.
  - **Sketch / ink lines from a colour dodge (`pow(grey(colorDodge(col, 1 - blur)), k)`) draw every faint dip: wrinkles, pores, skin texture.** Gate them by local darkness: `line = mix(1.0, line, smoothstep(T, 1.5*T, grey(blur) - grey(col)))` with T about 0.06. A line is drawn only where the pixel is clearly darker than its surroundings, it costs no samples, and it does not depend on resolution. Tested on a real unfiltered selfie (2026-09-13, sketch, anime cel and B&W manga shaders; the owner picked this variant): the forehead comes out clean, while eyes, brows, nose, mouth and the head outline are kept. REJECTED on the same image: a size gate (dodging against a small blur instead of the pixel) softens and fades all lines, and size plus contrast together removes eyes and most outlines. A colour-burn line blend hides faint lines only because bright skin swallows them; it cannot work on white-paper outputs (burn over white = no line at all).
  - **Removing a posterize step moves the thresholds**: for gray, `floor(c*8)/8 > T` means `c >= (floor(8T)+1)/8`. Apply that to the smoothed luminance, otherwise zones grow and the look changes.
...
- **Gestures**: `GesturesMode.Enabled` routes pointer/touch/wheel to DrawnUI and shares with the page like MAUI's `Enabled` in a native scroll view (`applyGestureStyle` sets the canvas `touch-action` to the axes the page can scroll, `none` when it cannot, and vertical always when the canvas is inside an iframe, so a finger pan along a page axis scrolls the page; `OnWheel` returns whether a control used the wheel and the page scrolls otherwise). `GesturesMode.Lock` keeps all input and additionally applies CSS guard at lib level (`applyGestureStyle` in `drawnui-web.js`: `touch-action:none; user-select:none` on canvas + `overscroll-behavior:none` on page + non-passive `touchmove` preventDefault) to stop iOS page panning / swipe-to-close — Web has no AppoMobi `TouchEffect`, so this CSS+JS guard replaces it.
...
## MeasureFirst is uniform cells BY DESIGN — self-sized stacks use MeasureAll (owner ruling 2026-09-10)

`MeasureFirst` (the templated default) = every cell takes the FIRST cell's size; the Add/Remove fast path (`TryApplyUniformAddMeasureFirst`) stamps new cells with the first cell's `Destination` and the draw-time measure of a fresh cell runs inside that `cell.Area`. A templated stack that takes its cross-axis size from its cells (auto-width Column with `HorizontalOptions=Start` holding auto-width labels; ArtOfFoto spot-meter results list) therefore squeezes a wider later item into the first item's width (text trails "...", stack never widens). That is NOT a lib bug: set `MeasureItemsStrategy = MeasuringStrategy.MeasureAll` on such a stack (small by nature). A lib "fix" gating the fast path on `NeedAutoWidth` was committed and REVERTED the same day: a raw `SkiaLayout Column` without `HorizontalOptions=Fill` is auto-width too, and large lists like that would have re-measured every cell on each append. Contract pinned by `src/Net/Tests/DrawnUi.Net.Tests/MeasureFirstAutoWidthTests.cs` (MeasureAll grows, MeasureFirst stays uniform). Repro notes: with `Split=0` the measure pass already measures every cell (`columnsCount != Split`), the uniform behaviour lives in the Add fast path — a bare `Measure()` test cannot show it, use the render loop (`HeadlessCanvasHost`, `ObservableCollection.Add` between `RenderFrame`s, `ChildrenFactory.GetCellInUseOrNull(i)` for a live cell). The shared `Tests/UnitTests` project only compiles with the stale `SkiaLayoutTests.cs` moved aside.
