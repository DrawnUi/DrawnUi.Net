...
Drop-in for `SkiaStack` in scrolling lists with many cells per screen: records viewport ±1 viewport into an SKPicture plane, blits it while scrolling, re-records per half-viewport drift or on invalidation. `UseDoubleBuffering=true` (default for chat) records the NEXT plane off-thread while the current blits — scroll never pays a render-thread record; on MAUI heads this requires `UsePreparedViews` (bake must be pure cache blits — MAUI BindableObject is not thread-safe).
...
- DIAGNOSING "is the plane actually working": `DebugString` prints `plane [coveredTop..coveredBot] valid=True, blit N% (M live), rec R/J` — `plane none` means the plane NEVER installed and every frame is a live per-cell draw. Second signal: on blit frames `DrawStack` doesn't run, so `drawn X-Y` stays FROZEN while the content moves. `drawn` ticking on every few-pixel scroll = not blitting.
- READING `blit N% (M live), rec R/J`: `M` = live per-cell frames, `R` = committed band records, `J` = records the completeness gates refused. A healthy image grid sits at blit 99% with a handful of live frames — **`blit%` being high does NOT mean the plane is cheap**, because a dirty frame does `CreateCache` + blit and still counts as a blit. `R` is the real cost number. Compare it against what drift alone justifies: `scrolled px / (PlaneRefreshRatio * viewportPx)`. Device measurement 2026-07-20 (DrawnCells VariantA, 2-col image grid, ~21000px scrolled, 2200px viewport → ~19 drift records): actual **96**. The excess is cell content arriving (images), not structure churn.
- Those excess records are NOT filterable. Verified on device: gating the blit path on "dirty cell is visible AND `CellRecordReady`" changed 96→95 records, i.e. nothing — the dirt IS actionable, each arriving image genuinely gives its cell new pixels. The only lever is BATCHING: `PlaneDirtyCoalesceFrames` (default 8) holds a DIRT-ONLY refresh for N frames so several arrivals collapse into one record. Never holds a structure change, a coverage miss or an ordered scroll, and flushes immediately once `_settledFrames >= 2`. Measured: 8 frames → 80 records (-17%), 24 frames → 71 (-26%), diminishing. Frame rate did NOT move on a 84fps device (an SKPicture ops record is cheap there) — the knob matters on weak hardware where a record dominates the frame, so treat the record count, not fps, as its signal.
