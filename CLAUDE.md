...
| ~~Mouse wheel does not accumulate~~ FIXED 2026-09-24 | `SkiaScroll.ApplyWheelScroll` | A notch arriving while the previous wheel scroll animates now adds onto that scroll's destination (`_scrollerX/Y.mMaxValue`), the React rule. Harness: 8 notches 40 ms apart went from 337 pt to 1200 pt; single notch and slow spins unchanged. Edge hand-off to an outer scroll and animator retarget were NOT ported | — |
